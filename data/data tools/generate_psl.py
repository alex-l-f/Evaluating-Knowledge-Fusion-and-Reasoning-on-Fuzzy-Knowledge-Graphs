...
#config
path = "./data/unprocessed/OpenEA/EN_DE_100K_V2/"
filenames = ["kg1_list.tsv", "kg2_list.tsv", "link_list.tsv", "kg_atts.tsv"]
...
for fold in range(1, 6):
    ...
    for filename in filenames:
        tup, rtup = load_triples(path + f"{fold}/" + filename)
        #combine or add to triples
        for h in tup.keys():
            if h not in triples:
                triples[h] = {}
            for t in tup[h].keys():
                if t not in triples[h]:
                    triples[h][t] = {}
                for r in tup[h][t].keys():
                    triples[h][t][r] = tup[h][t][r]
        #combine or add to r_triples
        for r in rtup.keys():
            if r not in r_triples:
                r_triples[r] = {}
            for h in rtup[r].keys():
                if h not in r_triples[r]:
                    r_triples[r][h] = {}
                for t in rtup[r][h].keys():
                    r_triples[r][h][t] = rtup[r][h][t]
    ...
    #compute hit rate for each rule, while keeping track of the number of occurances
    r_hit_rate_final = {}
    ...
    r_hit_rate_final_flat = list(filter(lambda x: x[3] >= 0.9 and r_count[x[0]][x[1]] > 100, r_hit_rate_final_flat))
    ...
    with open(path + f"{fold}/" + filenames[0].replace(".tsv","_psl.tsv"), "w") as f:
